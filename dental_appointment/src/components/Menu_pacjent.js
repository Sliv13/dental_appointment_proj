...
export default function Menu_glowne_pacjent() {
  const navigate = useNavigate();
  const [anchorEl, setAnchorEl] = React.useState(null);
  const open = Boolean(anchorEl);
  const zaplanowane = '/zaplanowane'
  const mainpage = '/pacjent'
  const wyszukiwarka = '/wyszukiwarka'
  const logout = '/logout'
  ...
  return (
    ...
    <Stack direction='row' spacing={2}>
      <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
        <Stack direction='row' spacing={2}>
        <Button variant='contained' onClick={()=>HandleNav(navigate,wyszukiwarka)} color='info' sx={{ minWidth: 100 }}>ZAPISZ SIĘ</Button>
